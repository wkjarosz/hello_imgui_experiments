...
vec4 sample_image(sampler2D sampler, vec2 uv, bool within_image)
{
...
void main()
{
    ...
    vec4 value = sample_image(primary_texture, primary_uv, in_img);

    if (has_reference)
    {
        vec4 reference_val = sample_image(secondary_texture, secondary_uv, in_ref);
