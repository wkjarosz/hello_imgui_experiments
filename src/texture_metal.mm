...
void Texture::init()
{
    ...
    sampler_desc.minFilter = m_min_interpolation_mode == InterpolationMode::Nearest ? MTLSamplerMinMagFilterNearest
                                                                                    : MTLSamplerMinMagFilterLinear;

    sampler_desc.magFilter = m_mag_interpolation_mode == InterpolationMode::Nearest ? MTLSamplerMinMagFilterNearest
                                                                                    : MTLSamplerMinMagFilterLinear;

    sampler_desc.mipFilter = m_min_interpolation_mode == InterpolationMode::Trilinear ? MTLSamplerMipFilterLinear
...
void Texture::upload(const uint8_t *data)
{
    ...
    [command_encoder copyFromTexture:temp_texture
                         sourceSlice:0
                         sourceLevel:0
...
void Texture::resize(const int2 &size)
{
    ...
    if (m_pixel_format == PixelFormat::BGRA && m_component_format != ComponentFormat::UInt8)
        m_pixel_format = PixelFormat::RGBA;
    ...
    default: throw std::runtime_error("Texture::Texture(): invalid pixel format!");
    }
    ...
    id<MTLDevice>         device       = gMetalGlobals.caMetalLayer.device;
    MTLTextureDescriptor *texture_desc = [MTLTextureDescriptor texture2DDescriptorWithPixelFormat:pixel_format_mtl
                                                                                            width:(NSUInteger)m_size.x
                                                                                           height:(NSUInteger)m_size.y
                                                                                        mipmapped:mipmap];
